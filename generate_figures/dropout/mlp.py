...
class HiddenLayer(object):
    def __init__(self, rng, input, n_in, n_out,
        ...
        if W is None:
            ...
            W_values = 2. * np.asarray(rng.standard_normal(
...
def f_df_split(fnc, *args, **kwargs):
    result = fnc(*args, **kwargs)
    return result[0], results[1:]

def build_f_df(layer_sizes, dropout=False, use_bias=False, rectifier=None):
    ...
    classifier = MLP(rng=rng, input=x,
           layer_sizes=layer_sizes, use_bias=use_bias, rectifier=rectifier)
...
#                y: test_set_y[index * batch_size:(index + 1) * batch_size]})
    ...
    symbolic_params = [convert_variable(param) for param in classifier.params]
    givens = dict(zip(classifier.params, symbolic_params))
    f_df = theano.function(inputs=symbolic_params + [x, y], outputs=[cost] + gparams, givens=givens)
    return f_df, classifier
...
if __name__ == '__main__':
    ...
    build_f_df(layer_sizes, use_bias=False)
